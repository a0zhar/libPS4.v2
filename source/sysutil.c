...
void initSysUtil(void) {
  if (!sysUtilHandle) {
    ...
    HOOKFUNCTION(sysUtilHandle, sceSysUtilSendSystemNotificationWithText);
  }
  ...
  if (!libSceSystemService) {
    ...
    HOOKFUNCTION(libSceSystemService, sceSystemServiceLaunchWebBrowser);
  }
}
...
void initUserService(void) {
  ...
  HOOKFUNCTION(libSceUserService, sceUserServiceInitialize);
  HOOKFUNCTION(libSceUserService, sceUserServiceGetInitialUser);
  HOOKFUNCTION(libSceUserService, sceUserServiceGetLoginUserIdList);
  HOOKFUNCTION(libSceUserService, sceUserServiceGetUserName);
  HOOKFUNCTION(libSceUserService, sceUserServiceTerminate);
}
...
void reboot() {
  ...
  HOOKFUNCTION(libkernel, sceKernelReboot);
  sceKernelReboot();
}
