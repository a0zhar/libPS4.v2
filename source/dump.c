...
SegmentBufInfo *parse_phdr(Elf64_Phdr *phdrs, int num, int *segBufNum) {
  SegmentBufInfo *infos = malloc(sizeof(SegmentBufInfo) * num);
  if (infos == NULL) return NULL; // Is this what should be returned when unable to allocate `infos`?
...
int wait_for_bdcopy(char *title_id) {
  ...
  // This is what triggers a "dump" when a game is deleted while the dumper is already running
  // Returning 100 will stop the wait_for_bdcopy loop
  ...
  if (buf == NULL) {
    fclose(pbm);
    // Return 0 on when unable to allocate buffer, should this be 100? 
    // It will trigger the same issue above
    return 0; 
  }
...
int wait_for_usb(char *usb_name, char *usb_path) {
  ...
  while (fd == -1) {
    ...
    // 10 attempts at each USB #, reaching 8 resets to 0
    if (row >= 80) {
      row = 0;
    } else {
      row += 1;
