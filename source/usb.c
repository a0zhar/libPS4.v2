...
void initUsb(void) {
  ...
  HOOKFUNCTION(libUsb, sceUsbdInit);
  HOOKFUNCTION(libUsb, sceUsbdExit);

  HOOKFUNCTION(libUsb, sceUsbdGetDeviceList);
  HOOKFUNCTION(libUsb, sceUsbdFreeDeviceList);

  HOOKFUNCTION(libUsb, sceUsbdGetDeviceDescriptor);

  HOOKFUNCTION(libUsb, sceUsbdOpen);
  HOOKFUNCTION(libUsb, sceUsbdOpenDeviceWithVidPid);
  HOOKFUNCTION(libUsb, sceUsbdClose);

  HOOKFUNCTION(libUsb, sceUsbdSetInterfaceAltSetting);
  HOOKFUNCTION(libUsb, sceUsbdClearHalt);
  HOOKFUNCTION(libUsb, sceUsbdResetDevice);
  HOOKFUNCTION(libUsb, sceUsbdCheckConnected);

  HOOKFUNCTION(libUsb, sceUsbdControlTransfer);
  HOOKFUNCTION(libUsb, sceUsbdBulkTransfer);
  HOOKFUNCTION(libUsb, sceUsbdInterruptTransfer);

  HOOKFUNCTION(libUsb, sceUsbdGetActiveConfigDescriptor);
  HOOKFUNCTION(libUsb, sceUsbdGetConfigDescriptor);
  HOOKFUNCTION(libUsb, sceUsbdGetConfigDescriptorByValue);
  HOOKFUNCTION(libUsb, sceUsbdFreeConfigDescriptor);
}
